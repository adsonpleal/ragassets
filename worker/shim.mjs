...
//
// Eight seconds, and the number is a measured trade rather than a guess.
//
// The cost of this mechanism lands entirely on the one request that waits it out
// before being re-served, so four seconds was tried, on the reasoning that
// renders never approach it — p95 630 ms, worst observed 1.37 s. It did halve
// that request's wait, from 8,546 ms to 4,181 ms. It also brought back twenty
// "Worker exceeded memory limit" exceptions per 360 renders, from zero.
//
// The link is direct: a shorter threshold fires more retries, every retry
// condemns an instance, and a condemned instance cannot be freed while the
// goroutines parked inside it keep the runtime reachable. More retries means more
// runtimes coexisting in one 128 MiB isolate.
//
// So the two are in tension, and an OOM is the worse end of it — it takes down
// everything in flight, where the timeout costs one request some seconds. Eight
// stays, until the leak itself is fixed rather than budgeted around.
